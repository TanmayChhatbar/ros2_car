...
sv = symvar(eqn);
idx_targets = string(sv) == "vx_target" | string(sv) == "vy_target";
uv = sv(idx_targets)
sv = sv(~idx_targets)
...
wz = 0;
delta = 0;
w_front = solveFrontWheelSpeeds(t_vx, t_vy, wz, delta, v3.config.r_wheel, eqn_vsub(2:3));

eqn_sub = subs(eqn_vsub, 'w_wheel1', w_front(1));
eqn_sub = subs(eqn_sub, 'w_wheel2', w_front(2));

%%
f2 = matlabFunction(lhs(eqn_sub([1 4 5])));
w_r_kinematic = vx_target / v3.config.r_wheel;
if 1
    options = optimoptions("fsolve", 'MaxFunctionEvaluations', 1e3);
    f2b = @(x) f2(x(1), x(2), x(3));
    [soln, val] = fsolve(f2b, [t_vx, t_vy, w_r_kinematic*1.1], options);
    val = norm(val)
else
    f2b = @(x) norm(f2(x(1), x(2), x(3)));
    [soln, val] = fmincon(f2b, [t_vx, t_vy, w_r_kinematic*1.1]);
    val = norm(val)
end
r_delta = soln(1);
r_w_yaw = soln(2);
r_w_wheel_rear = soln(3);
