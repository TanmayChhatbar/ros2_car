...
void stabilizeWheelSpeeds(Vehicle2D_ST &vehicle)
{
    // stabilize wheel speeds (to solve equilibrium for non-driven wheels)
    // get data
    Vehicle2D_STData &data = vehicle.getVehicle2D_STData();
    const Vehicle2D_STConfig &config = vehicle.getVehicle2D_STConfig();
    ...
    double vxw[2] = {vx, vx};
    double vyw[2] = {vy + a * w_yaw, vy - b * w_yaw};
    double w_wheel[2];

    vxw[0] = vxw[0] * std::cos(steering_angle) + vyw[0] * std::sin(steering_angle);
    w_wheel[0] = vxw[0] / r_wheel;

    w_wheel[1] = vxw[1] / r_wheel;

    // set data
    const double wheel_torques[2] = {0.0, 0.0};
...
double cost_function(const std::vector<double> &x, std::vector<double> &grad, void *f_data)
{
    ...
    // load data
    Vehicle2D_ST &vehicle = *static_cast<Vehicle2D_ST *>(f_data);
    Vehicle2D_STData &data = vehicle.getVehicle2D_STData();
    const Vehicle2D_STConfig &config = vehicle.getVehicle2D_STConfig();
    ...
    if (config.getDrivetrainType() == DrivetrainType_E::RWD)
    {
        ...
        w_wheel[1] = x[0]; // only rear wheels are driven
    }
    else if (config.getDrivetrainType() == DrivetrainType_E::FWD)
    {
        w_wheel[0] = x[0];
        w_wheel[1] = w_wheel_eq[1]; // only front wheels are driven
    }
    else if (config.getDrivetrainType() == DrivetrainType_E::AWD)
    {
        w_wheel[0] = x[0];
        w_wheel[1] = x[0]; // all wheels are driven
    }
    else
    {
...
double optimize(Vehicle2D_ST &vehicle, double vx_target, double vy_target)
{
    // get data and config from vehicle object
    Vehicle2D_STData &data = vehicle.getVehicle2D_STData();
    const Vehicle2D_STConfig &config = vehicle.getVehicle2D_STConfig();

    const double max_yaw_rate = 10.0; // [rad/s] maximum yaw rate
    ...
    std::vector<double> lb = {
        ...
        DEG2RAD(-85.0), // [rad] steering angle
        -max_yaw_rate}; // [rad/s] yaw rate
    std::vector<double> ub = {
        wheel_speed_at_vx * 500.0,
    ...
    auto f = [&vehicle](const std::vector<double> &x)
    {
        ...
        Vehicle2D_ST vehicle_copy = vehicle;
