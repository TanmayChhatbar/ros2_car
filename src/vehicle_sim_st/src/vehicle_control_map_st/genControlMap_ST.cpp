#include "genControlMap_ST.hpp"

int main()
{
    // load configurations
    const Vehicle2D_STConfig config = Vehicle2D_STConfig::loadFromFile("../configs/tt02.json");
    ...
    const double steering_angle_threshold = DEG2RAD(85); // [rad]

    const bool refine_for_wheel_speed = false;
    const double wheel_speed_ratio_threshold = 58.0 / config.getWheelRadius(); // [(rad/s)/(m/s)]
    ...
    return 1;
#endif
    const std::string filename = "../../control_map_st.csv";
...
#pragma omp parallel for
    ...
    {
        Vehicle2D_ST vehicle = Vehicle2D_ST(config);
